...
class SmokeSRTests(unittest.TestCase):
    ...
    def test_58_g1_wrapper_remediation_preserves_frozen_source_binding(self):
        generator = (ROOT / "journal_extension/kaggle/generate_canonical_notebook.py").read_text()
        expected = "3a90234f66ee09ed25141d5c45c7ed38971d69e5"
    ...
    def test_78_operator_docs_are_synchronized_for_fresh_v22_g1(self):
        ...
        g1_section = guide.split("### `g1`", 1)[1].split("### `calibration-dual`", 1)[0]
        launch_block = g1_section.split("```text", 1)[1].split("```", 1)[0]
        self.assertIn("CROPCOP_G1_ALLOW_CREATE_PRIVATE_DATASET=0", launch_block)
        self.assertNotIn("CROPCOP_G1_ALLOW_CREATE_PRIVATE_DATASET=1", launch_block)
