...
class TrackAV12MasterV6Tests(unittest.TestCase):
    ...
    def test_singleton_terminal_marker_prevents_second_execution_in_same_session(self):
        with tempfile.TemporaryDirectory() as td:
            with unittest.mock.patch.dict(os.environ, {"CROPCOP_MASTER_SINGLETON_ROOT": td}, clear=False):
    ...
    def test_concurrent_duplicate_waits_and_mirrors_primary_result(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            child = subprocess.Popen([sys.executable, "-c", code], env=env, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            time.sleep(0.5)
