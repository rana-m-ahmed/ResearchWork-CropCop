...
def _run_owner(account_id: str, *, lock_path: Path, status_path: Path, runtime_sha: str) -> int:
    ...
    print(
        f"MASTER_LAUNCH_GUARD_V12 OWNER account={account_id} pid={os.getpid()} ppid={os.getppid()} "
        f"launch={launch_id[:12]} science={SCIENCE_SHA[:12]} runtime={runtime_sha[:12]}",
        flush=True,
    )
    driver = Path(__file__).resolve().parent / "master_account_driver_v12.py"
    try:
    ...
    print(f"MASTER_LAUNCH_GUARD_V12 OWNER_FINISHED account={account_id} rc={rc} launch={launch_id[:12]}", flush=True)
    if exc_text is not None:
...
def launch_or_follow(account_id: str) -> int:
    ...
    guard_root = Path(os.environ.get("CROPCOP_MASTER_GUARD_ROOT", "/kaggle/working/.cropcop_tracka_master_guard_v12"))
        ...
        while True:
            try:
                ...
                if prior_code is not None:
                    print(
                        f"MASTER_LAUNCH_GUARD_V12 TERMINAL_REUSE account={account_id} rc={prior_code} "
                ...
                if status and status.get("state") == "RUNNING":
                    if (
                        ...
                        raise OperatorError("foreign/stale RUNNING marker conflicts with current v12 science/runtime/account identity; use a fresh Batch session")
                    raise OperatorError("stale RUNNING marker found after lock acquisition; use a fresh Kaggle Batch session")
                if observed_busy:
                    print("MASTER_LAUNCH_GUARD_V12 prior owner released without matching terminal marker; taking ownership only because no canonical terminal result exists.", flush=True)
                return _run_owner(account_id, lock_path=lock_path, status_path=status_path, runtime_sha=runtime_sha)
            except BlockingIOError:
                if not observed_busy:
                    observed_busy = True
                    print(f"MASTER_LAUNCH_GUARD_V12 DUPLICATE_DETECTED account={account_id}; following canonical owner.", flush=True)
...
def main() -> int:
    if len(sys.argv) != 2:
        raise SystemExit("usage: master_launch_guard_v12.py K1|K2|K3")
