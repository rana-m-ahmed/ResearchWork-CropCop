...
import master_science as v3_science
from master_publication_v4 import publish_public_files
...
ensure_science_durability = v3_science.ensure_science_durability
...
def science_command(
    ...
    *,
    ...
    """Invoke the frozen scientific runner without its non-idempotent Git publisher."""
    return [
        sys.executable,
...
def publish_completed_scientific_runs(repo: Path, account_id: str, summary: dict, master_root: Path) -> dict[str, dict]:
    """Publish only terminal PASS scientific runs from the parent after frozen execution."""
    ...
    for slot_id, rows in (summary.get("slot_results") or {}).items():
        ...
        for row in rows:
            ...
            if not candidates:
                raise OperatorError(
                    f"terminal PASS scientific run has no public evidence candidates: {experiment_id}"
                )
