...
from tracka_v12_kaggle_operator_v8 import SCIENCE_SHA, OperatorError, load_github_token
...
def _fetch_remote_ref(repo: Path, run_id: str) -> str | None:
    ...
    cp = subprocess.run(
        ["git", "-C", str(repo), "fetch", "--quiet", "--force", "origin", f"refs/heads/{branch}:{remote_ref}"],
...
def _assert_source_ancestry(repo: Path, remote_ref: str) -> None:
    ...
    if cp.returncode != 0:
        raise OperatorError(f"existing public evidence branch does not descend from v8 science SHA {SCIENCE_SHA}: {remote_ref}")
...
def _publish_one(repo: Path, run_id: str, path: Path, frozen_publish, *, attempts: int) -> str:
    branch = f"run-evidence/{run_id}"
    ...
    for attempt in range(1, attempts + 1):
        if remote_evidence_matches(repo, run_id, [path]):
            return branch
        try:
            result = frozen_publish(repo_dir=repo, source_git_sha=SCIENCE_SHA, run_id=run_id, files=[str(path)])
            if not remote_evidence_matches(repo, run_id, [path]):
                raise OperatorError(f"public evidence file did not round-trip after publish: {branch}/{path.name}")
            return result
        except Exception as exc:
            last_error = exc
            try:
                if remote_evidence_matches(repo, run_id, [path]):
                    print(f"Public evidence became canonical despite client-side publish error: {branch}/{path.name}")
                    return branch
            ...
            print(f"public evidence publication attempt {attempt}/{attempts} failed for {run_id}/{path.name}: {type(last_error).__name__}; retrying in {delay}s")
            time.sleep(delay)
    assert last_error is not None
    raise last_error


def publish_public_files(repo: str | Path, run_id: str, files: list[str | Path], *, attempts: int = 3) -> str:
    """Idempotently publish audited public-safe evidence from the exact v8 science ancestry."""
    if attempts < 1:
...
def install_stage_publication_hooks() -> None:
    """Route every v8 pre-science stage through the v8 idempotent publisher."""
    import master_g1a_v8
    import master_g2a_v8
    import master_control_v8

    master_g1a_v8.publish_public_files = publish_public_files
    master_g2a_v8.publish_public_files = publish_public_files
    master_control_v8.publish_public_files = publish_public_files
