...
from master_continuation_v10 import try_acquire_control_worker, try_collect_all_g2a
...
def guard_identity(account_id: str) -> str:
    ...
    if len(token) < 16 or guard_account != account_id or not lock_path:
        raise OperatorError("v10 master driver must be launched through master_launch_guard_v10.py")
    return token


def stage(name: str, account_id: str, guard_token: str) -> None:
    print(
        f"\n=== TRACKA_V12_MASTER_V10 {account_id} :: {name} "
...
def controlled_dependency_continuation(account_id: str, reason: str) -> int:
    ...
    print("Current account evidence is safely published. End this Batch session and rerun the SAME v10 notebook later.", flush=True)
    return 2


def main() -> int:
    if len(sys.argv) != 2 or sys.argv[1] not in MASTER_ACCOUNTS:
        raise SystemExit("usage: master_account_driver_v10.py K1|K2|K3")
