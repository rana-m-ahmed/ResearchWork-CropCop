...
def validate_control_bundle(repo: Path, g1a_seal: dict, control_dir: Path) -> dict:
    ...
    errors = validate_g2a_v122_barrier(
        barrier, expected_source_sha=SCIENCE_SHA,
        expected_g1a_seal_sha256=g1a_seal["g1a_seal_sha256"],
    )
    errors += validate_scheduler_freeze_v122(
        scheduler, expected_g2a_barrier_sha256=barrier["barrier_sha256"],
    )
...
def build_control_k1(
    repo: Path, *, g1a_bundle: Path, g1a_seal: dict, summaries: dict[str, Path],
    master_root: Path, stack: dict,
) -> tuple[Path, dict]:
    ...
    command = [
        ...
        "--g1a-seal", str(g1a_bundle / "TRACKA_V12_G1A_SEAL.json"),
        "--g2a-barrier", str(barrier), "--scheduler-freeze", str(scheduler),
    ...
    control = validate_control_bundle(repo, g1a_seal, control_dir)
    report = {
        "schema_version": "1.0", "stage": "TRACKA_V12_MASTER_CONTROL", "status": "PASS",
...
def acquire_control_worker(repo: Path, *, g1a_seal: dict, master_root: Path) -> tuple[Path, dict]:
    ...
    while True:
        ...
        if time.monotonic() - started >= MASTER_WAIT_MAX_SECONDS:
            raise TimeoutError("timed out waiting for K1 canonical Track-A control plane")
        time.sleep(30)
