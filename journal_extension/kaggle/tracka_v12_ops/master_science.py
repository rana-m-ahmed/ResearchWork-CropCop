...
from tracka_v12_kaggle_operator_v3 import (
    SCIENCE_SHA, OperatorError, account_public_run_id, ensure_private_dataset, load_json,
    operator_runtime_head, publish_public_files, technical_rollover_only,
    validate_private_locators_with_science, wait_kaggle_dataset_ready, write_json,
)
...
def science_command(
    repo: Path, *, account_id: str, manifest: Path, class_map: Path, image_root: Path,
    g1a_bundle: Path, control_dir: Path, master_root: Path,
) -> list[str]:
    return [
        sys.executable, str(repo / "journal_extension/kaggle/run_tracka_v12_account.py"),
        "--account-id", account_id, "--repo-root", str(repo), "--source-git-commit", SCIENCE_SHA,
        ...
        "--output-root", str(master_root / "science"), "--num-workers", "4",
        "--checkpoint-every-steps", "250", "--session-hard-limit-seconds", str(12 * 3600),
        "--finalization-margin-seconds", "3600", "--minimum-new-run-safe-seconds", "1800",
        "--min-free-gb", "5", "--publish-evidence",
    ]


def sanitized_account_report(account_id: str, summary: dict, control: dict) -> dict:
    slots = {}
    for slot_id, rows in (summary.get("slot_results") or {}).items():
        slots[slot_id] = [
            {
                "experiment_id": row.get("experiment_id"), "run_id": row.get("run_id"),
                "return_code": row.get("return_code"), "run_status": row.get("run_status"),
                "continuation_required": row.get("continuation_required"),
                "selected_checkpoint_sha256": row.get("selected_checkpoint_sha256"),
                "publication_status": (row.get("publication") or {}).get("status"),
                "publication_branch": (row.get("publication") or {}).get("branch"),
            }
            for row in rows
        ]
    return {
        "schema_version": "1.0", "stage": "TRACKA_V12_MASTER_ACCOUNT", "account_id": account_id,
        "status": summary.get("status"), "science_complete": summary.get("science_complete"),
        ...
        "slot_results": slots, "execution_errors": summary.get("execution_errors", []),
        "publication_repairs": summary.get("publication_repairs", []),
...
def run_science(
    repo: Path, *, account_id: str, manifest: Path, class_map: Path, image_root: Path,
    g1a_bundle: Path, control_dir: Path, control: dict, master_root: Path,
) -> int:
    command = science_command(
        repo, account_id=account_id, manifest=manifest, class_map=class_map, image_root=image_root,
        g1a_bundle=g1a_bundle, control_dir=control_dir, master_root=master_root,
    )
    ...
    write_json(public_report, sanitized_account_report(account_id, summary, control))
    account_report_published = True
    try:
        publish_json(repo, account_public_run_id(account_id), public_report)
    except Exception as exc:
        account_report_published = False
    ...
    if cp.returncode == 0:
        ...
        if not account_report_published:
            print(f"{account_id}: science complete; account report publication needs repair. Rerun SAME master notebook.")
            return 2
        print(f"{account_id}: SCIENTIFIC QUEUE TERMINAL PASS")
        return 0
    if cp.returncode == 2:
        if technical_rollover_only(summary):
            print(f"{account_id}: planned session rollover. Rerun this SAME master notebook in a fresh Batch session.")
            return 2
        if summary.get("science_complete") is True and summary.get("publication_repairs"):
            print(f"{account_id}: science complete; GitHub evidence publication needs repair. Rerun SAME master notebook.")
            return 2
    raise OperatorError(
