...
def build_g1a_once(repo: Path, *, manifest: Path, class_map: Path, image_root: Path, output_bundle: Path) -> dict:
    ...
    command = [
        ...
        str(principal),
        "--effb0-pretrained",
        str(baselines["effb0"]["artifact"]),
        "--effb0-provenance",
        str(baselines["effb0"]["provenance"]),
        "--cnxtt-pretrained",
        str(baselines["cnxtt"]["artifact"]),
        "--cnxtt-provenance",
        str(baselines["cnxtt"]["provenance"]),
        "--r13-pretrained",
        str(r13),
        "--bundle-dir",
        str(output_bundle),
...
def _publish_failed_handoff_best_effort(repo: Path, handoff: Path, locator: str) -> None:
    write_handoff(
        ...
        status="FAILED",
        locator=locator,
        failure_code=G1A_FAILURE_CODE,
    )
    try:
        publish_json(repo, g1a_handoff_run_id(), handoff)
    except Exception as publish_exc:
        print(
            "G1A_FAIL_HANDOFF_PUBLICATION_FAILED "
            f"{type(publish_exc).__name__}; preserving original K1 failure and failing closed.",
            flush=True,
        )
    ...
    if adopted is not None:
        ...
        try:
            ...
            version_private_dataset(
                locator,
                bundle,
                message=f"Canonical Track-A v1.2 G1A {SCIENCE_SHA[:12]}",
                env=kaggle_env,
            )
    ...
    while True:
        ...
        if path is not None:
            ...
            if (
                ...
                and payload.get("operator_runtime_sha") == runtime_sha
            ):
                ...
                if status == "FAILED":
                    ...
                    raise OperatorError(
                        "K1 reported canonical G1A failure for this exact science/runtime identity; "
                        "K2/K3 will not wait for G2A/control. Repair K1 and rerun all masters fresh."
                    )
                if status == "READY" and len(str(payload.get("g1a_seal_sha256", ""))) == 64:
                    return payload
        if time.monotonic() - started >= MASTER_WAIT_MAX_SECONDS:
            raise TimeoutError("timed out waiting for current-runtime canonical G1A handoff")
        time.sleep(30)


def acquire_canonical_g1a_worker(
    repo: Path,
    *,
    kaggle_env: dict[str, str],
    master_root: Path,
) -> tuple[Path, dict, str]:
    handoff = _wait_for_current_g1a_handoff(
        repo,
        master_root / "handoff" / G1A_HANDOFF_FILE,
    )
    locator = str(handoff["private_kaggle_dataset_locator"])
    print(f"Waiting for canonical G1A private access: {locator}")
    wait_kaggle_dataset_ready(locator, env=kaggle_env)
    bundle, seal = download_and_validate_g1a_dataset(
        repo,
        locator,
        master_root / "g1a-shared-download",
        env=kaggle_env,
    )
