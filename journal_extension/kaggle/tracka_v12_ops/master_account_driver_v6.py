...
from master_g1a_v6 import acquire_canonical_g1a_worker, ensure_canonical_g1a_k1
...
def run_account(account_id: str) -> int:
...
def _parse_account() -> str:
    if len(sys.argv) != 2 or sys.argv[1] not in MASTER_ACCOUNTS:
        raise SystemExit("usage: master_account_driver_v6.py K1|K2|K3")
    return sys.argv[1]


def cli() -> int:
    account_id = _parse_account()
    lease = acquire_master_execution(account_id)
    if lease.mirror_code is not None:
        return lease.mirror_code

    try:
        code = int(run_account(account_id))
    except TimeoutError as exc:
        detail = f"{type(exc).__name__}: {exc}"
        lease.finish(2, detail)
        ...
        detail = f"{type(exc).__name__}: {exc}"
        lease.finish(1, detail)
        traceback.print_exc()
        return 1

    lease.finish(code, "account queue returned normally")
    return code
