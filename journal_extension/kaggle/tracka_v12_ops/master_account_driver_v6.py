...
def guard_identity(account_id: str) -> str:
    ...
    if len(token) < 16 or guard_account != account_id or not lock_path:
        raise OperatorError(
            "v6 master driver must be launched through master_launch_guard_v6.py; "
            "direct execution is refused to prevent duplicate account orchestration"
        )
    return token
...
def stage(name: str, account_id: str, guard_token: str) -> None:
    print(
        f"\n=== TRACKA_V12_MASTER {account_id} :: {name} "
        f"pid={os.getpid()} launch={guard_token[:12]} ===",
        flush=True,
    )
