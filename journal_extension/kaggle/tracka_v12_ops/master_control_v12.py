...
def compose_gate_inputs_v12(
    ...
    *,
    ...
    dynamic_gates = {
        "immutable_v12_lock",
        AUTH_RUNTIME_QUALIFICATION_GATE,
    ...
    if lock_runtime_attestation.get("immutable_v12_lock") != "PASS":
        errors.append("lock/runtime attestation gate is not PASS: immutable_v12_lock")
    if lock_runtime_attestation.get(LOCK_RUNTIME_QUALIFICATION_FIELD) != "PASS":
        errors.append("lock/runtime attestation gate is not PASS: " + LOCK_RUNTIME_QUALIFICATION_FIELD)
    if lock_runtime_attestation.get("candidate_claim_boundary_lock") != "PASS":
        errors.append("lock/runtime attestation gate is not PASS: candidate_claim_boundary_lock")
    ...
    if content_report.get("overall_status") != "PASS" or (content_report.get("static") or {}).get("status") != "PASS":
    ...
    if durability_errors:
        errors.extend(f"G2A durability: {error}" for error in durability_errors)
    ...
    bindings[AUTH_RUNTIME_QUALIFICATION_GATE] = {
        "kind": "exact_head_lock_runtime_attestation",
        "attestation_sha256": lock_runtime_attestation["attestation_sha256"],
    ...
    bindings["g2a"] = {
        ...
        "durability_contract_sha256": durability["durability_contract_sha256"],
    ...
    if set(gates) != set(REQUIRED_PRE_SCIENCE_GATES):
        raise AssertionError("v12 GO gate inventory construction drift")
    if set(bindings) != set(REQUIRED_PRE_SCIENCE_GATES):
        raise AssertionError("v12 GO binding inventory construction drift")
...
def seal_science_go_v12(
    repo: Path,
    *,
    code_attestation_path: Path,
    lock_attestation_path: Path,
    g1a_path: Path,
    g2a_path: Path,
    scheduler_path: Path,
    output_path: Path,
    ...
    if source != SCIENCE_SHA:
        raise OperatorError(f"v12 control bridge requires exact frozen science SHA {SCIENCE_SHA}; observed {source}")
    paths = {
        "code_attestation": code_attestation_path,
        "lock_runtime_attestation": lock_attestation_path,
        "g1a": g1a_path,
        "g2a": g2a_path,
    ...
    validation_errors = validate_science_authorization(
        authorization,
...
def build_control_k1(
    ...
    *,
    ...
    scheduler = control_dir / "TRACKA_V12_SCHEDULER_FREEZE.json"
    command = [sys.executable, str(repo / "journal_extension/scripts/seal_tracka_v12_g2a.py")]
    for calibration_id in sorted(legacy.REQUIRED_G2A):
    ...
    code_attestation, lock_attestation = legacy.materialize_verified_attestations(attestation_dir)
    science_go = control_dir / "TRACKA_V12_SCIENCE_GO.json"
    compatibility_path = control_dir / "TRACKA_V12_CONTROL_SCHEMA_COMPAT_V12.json"
    compatibility = seal_science_go_v12(
        repo,
        code_attestation_path=code_attestation,
        lock_attestation_path=lock_attestation,
        g1a_path=g1a_bundle / "TRACKA_V12_G1A_SEAL.json",
        g2a_path=barrier,
        scheduler_path=scheduler,
        output_path=science_go,
        compatibility_path=compatibility_path,
    )

    owners = {
        "K1": legacy.owner_from_summary(summaries["CAL-EFFB0"]),
        "K2": legacy.owner_from_summary(summaries["CAL-MNV4-LOGITS"]),
        "K3": legacy.owner_from_summary(summaries["CAL-R13"]),
    }
    if legacy.owner_from_summary(summaries["CAL-CNXTT"]) != owners["K1"]:
        raise OperatorError("K1 G2A summaries disagree on account owner")
    if legacy.owner_from_summary(summaries["CAL-MNV4-FEATURE"]) != owners["K2"]:
        raise OperatorError("K2 G2A summaries disagree on account owner")

    scheduler_payload = legacy.load_json(scheduler)
    durable_map = legacy.scientific_durable_map(scheduler_payload, owners)
    legacy.write_json(control_dir / "TRACKA_V12_DURABLE_MAP.json", durable_map)
    legacy.write_json(control_dir / "TRACKA_V12_ACCOUNT_OWNERS.json", owners)
    control = legacy.validate_control_bundle(repo, g1a_seal, control_dir)
    report = {
        "schema_version": "1.2.2",
        "stage": "TRACKA_V12_MASTER_CONTROL",
        "status": "PASS",
        "science_source_sha": SCIENCE_SHA,
        "operator_runtime_sha": legacy.operator_runtime_head(),
        "dependency_lock_sha256": stack["dependency_lock_sha256"],
        "g1a_seal_sha256": g1a_seal["g1a_seal_sha256"],
        ...
        "control_schema_compatibility": compatibility,
        "protected_test_accessed": False,
        "external_surface_accessed": False,
    }
    legacy.write_json(control_dir / "TRACKA_V12_CONTROL_PUBLIC_REPORT.json", report)
    publish_paths = [control_dir / name for name in CONTROL_FILES]
    publish_paths.append(compatibility_path)
    legacy.publish_public_files(repo, legacy.control_public_run_id(), publish_paths)
    if legacy.fetch_public_bundle(repo, legacy.control_public_run_id(), CONTROL_FILES, control_dir) is None:
