...
# Import v8 authority first. It patches inherited operator modules before any helper imports them.
...
from master_control_v8 import acquire_control_worker, build_control_k1
...
def guard_identity(account_id: str) -> str:
    ...
    if len(token) < 16 or guard_account != account_id or not lock_path:
        raise OperatorError("v8 master driver must be launched through master_launch_guard_v8.py")
    return token


def stage(name: str, account_id: str, guard_token: str) -> None:
    print(
        f"\n=== TRACKA_V12_MASTER_V8 {account_id} :: {name} "
...
def main() -> int:
    if len(sys.argv) != 2 or sys.argv[1] not in MASTER_ACCOUNTS:
        raise SystemExit("usage: master_account_driver_v8.py K1|K2|K3")
    ...
    if account_id == "K1":
        ...
        g1a_bundle, g1a_seal, shared_locator = acquire_canonical_g1a_worker(repo, kaggle_env=kaggle_env, master_root=master_root)
    ...
    if account_id == "K1":
        ...
        control_dir, control = build_control_k1(repo, g1a_bundle=g1a_bundle, g1a_seal=g1a_seal, summaries=summaries, master_root=master_root, stack=stack)
...
if __name__ == "__main__":
    try:
        ...
        print(f"CONTROLLED_DEPENDENCY_CONTINUATION: {exc}")
