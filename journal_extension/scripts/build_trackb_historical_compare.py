...
def _read_safe_development_rows(path: Path) -> list[tuple[str, str]]:
    require_sha256(path, DATASET_MANIFEST_SHA256, "frozen Final-V1 manifest")
    ...
    with path.open(encoding="utf-8", newline="") as fh:
        ...
        if missing:
            raise TrackBError(f"frozen Final-V1 manifest missing columns: {sorted(missing)}")
        for raw in reader:
            split = str(raw["split"]).strip()
            if split not in {"train", "val"}:
                continue
            hist_id = str(raw["record_key"]).strip()
            rel = str(raw["portable_relpath"]).strip().replace("\\", "/")
            posix = PurePosixPath(rel)
            if not hist_id or hist_id in ids:
                raise TrackBError(f"development surface has missing/duplicate stable ID: {hist_id!r}")
            if not rel or posix.is_absolute() or ".." in posix.parts or rel in rels:
                raise TrackBError(f"development surface has unsafe/duplicate path: {rel!r}")
            if not rel.startswith(split + "/"):
                raise TrackBError(f"development path/split mismatch: split={split!r}, path={rel!r}")
            if "test" in {part.lower() for part in posix.parts}:
                raise TrackBError(f"forbidden V1-test marker in development path: {rel}")
            ids.add(hist_id)
            rels.add(rel)
            counts[split] += 1
            rows.append((hist_id, rel))
    if counts != {"train": TRAIN_ROWS, "val": VAL_ROWS} or len(rows) != SAFE_ROWS:
        raise TrackBError(
            f"safe historical surface mismatch: expected train={TRAIN_ROWS}, val={VAL_ROWS}, "
            f"total={SAFE_ROWS}; got {counts}, total={len(rows)}"
        )
    return rows
...
def _pack_orb(rows, image_root: Path, out: Path, *, workers: int, chunk_size: int):
    ...
    with manifest_path.open("w", encoding="utf-8", newline="") as mf:
        writer = csv.DictWriter(
            mf,
            fieldnames=["hist_id", "raw_sha256", "phash64", "dhash64", "width", "height"],
        )
        writer.writeheader()
        for chunk_index, start in enumerate(range(0, len(rows), chunk_size)):
            ...
            def process(item):
                ...
                path = (image_root / rel).resolve()
                if image_root not in path.parents and path != image_root:
                    raise TrackBError(f"development image path escapes root: {rel}")
                if not path.is_file():
            ...
            xy = (
                np.concatenate([row[2]["xy"] for row in computed], axis=0)
                if counts.sum()
                else np.empty((0, 2), np.float32)
            )
            desc = (
                np.concatenate([row[2]["desc"] for row in computed], axis=0)
                if counts.sum()
    ...
    xy_mm = np.lib.format.open_memmap(
        out / "orb_xy.npy", mode="w+", dtype=np.float32, shape=(total, 2)
    )
    desc_mm = np.lib.format.open_memmap(
        out / "orb_desc.npy", mode="w+", dtype=np.uint8, shape=(total, 32)
    )
    shapes_mm = np.lib.format.open_memmap(
        out / "orb_shapes.npy", mode="w+", dtype=np.int32, shape=(len(rows), 2)
    )
    point_cursor = 0
    row_cursor = 0
...
def _encode_dino(rows, image_root: Path, model, out: Path, *, device: str, batch_size: int):
    ...
    dest = np.lib.format.open_memmap(
        out / "dino_features.npy",
        mode="w+",
        dtype=np.float32,
        shape=(len(rows), FEATURE_WIDTH),
    )
    with torch.no_grad():
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description=(
            "Build the safe Track-B historical comparison package from the frozen V1 "
            "development surface only (train+validation; 92,744 images)."
        )
    )
    ...
    if not image_root.is_dir():
        raise TrackBError(f"V1 image root missing: {image_root}")
    ...
    _encode_dino(
        rows, image_root, model, out, device=args.device, batch_size=args.dino_batch_size
    )
    ...
    certificate = {
        ...
        "coverage_scope": SAFE_SCOPE,
        "image_count": len(rows),
        "train_image_count": TRAIN_ROWS,
        "validation_image_count": VAL_ROWS,
        "v1_test_image_bytes_accessed": False,
        "ext_i_eligible": False,
        "maximum_evidence_grade": "EXT-S",
        "reason_for_partial_surface": (
            "Consumed V1-test image bytes remain closed. The executable post-closure builder "
            "indexes only the frozen development surface. EXT-I requires recovery of a complete "
            "pre-test cryptographically bound historical comparison representation."
        ),
        "source_manifest_sha256": sha256_file(v1_manifest),
        ...
        "orb": {"max_side": 800, "nfeatures": 1200, "total_keypoints": total_keypoints},
    ...
    files["build_certificate"] = {
        "path": "BUILD_CERTIFICATE.json",
        "sha256": sha256_file(out / "BUILD_CERTIFICATE.json"),
        "bytes": (out / "BUILD_CERTIFICATE.json").stat().st_size,
    }
    manifest = {
        "schema_version": "1.1",
